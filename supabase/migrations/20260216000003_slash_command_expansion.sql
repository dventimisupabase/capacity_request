-- Migration 14: Slash Command Expansion
-- Adds /capacity help, /capacity list, /capacity view <id> subcommands.
-- Changes empty /capacity to return help instead of bare usage.
...
BEGIN
  ...
  IF params ? 'command' THEN
    ...
    IF cmd_parts[1] = 'view' THEN
      ...
      IF kit IS NULL THEN
        -- Request has no slack channel, return text-only
        RETURN json_build_object(
          'response_type', 'ephemeral',
