-- Update WEB_APP_BASE_URL to point to PostgREST RPC endpoint
-- and fix build_block_kit_message to use hash-fragment routing
-- Delete old secret and recreate with new URL (using vault API for permissions)
DELETE FROM vault.decrypted_secrets WHERE name = 'WEB_APP_BASE_URL';
SELECT vault.create_secret(
  'https://oandzthkyemwojhebqwc.supabase.co/rest/v1/rpc/www?page=detail.html',
  'WEB_APP_BASE_URL'
);

-- Update build_block_kit_message to use hash fragment for request ID
-- The web_base_url now contains the full URL up to the page param,
-- so we just append #id=<request_id>
  ...
  IF web_base_url IS NOT NULL AND web_base_url != '' THEN
    actions := actions || jsonb_build_array(
      jsonb_build_object(
        ...
        'url', format('%s#id=%s', web_base_url, req.id),
        'action_id', 'view_web'
      )
    );
  END IF;
