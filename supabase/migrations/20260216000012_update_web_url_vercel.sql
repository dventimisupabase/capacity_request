-- Update WEB_APP_BASE_URL to point to Vercel-hosted web app
-- and revert build_block_kit_message to use query params (not hash fragments)
DELETE FROM vault.decrypted_secrets WHERE name = 'WEB_APP_BASE_URL';
SELECT vault.create_secret(
  'https://capreq.vercel.app',
  'WEB_APP_BASE_URL'
);

-- Revert build_block_kit_message URL format to query params
-- Vercel serves real HTML so detail.html?id=X works natively
